...
// Calculate activation fee based on earnings
function getActivationFee(balance: number): number {
  if (balance <= 500) return 150;
  if (balance <= 1000) return 250;
  if (balance <= 2000) return 350;
  if (balance <= 3000) return 400;
  if (balance <= 5000) return 500;
  return 700; // For earnings above 5000
}

const ActivateAccountModal = ({ isOpen, onClose, onActivated }: ActivateAccountModalProps) => {
  const { profile, refreshProfile, balance } = useAuth();
  const { activateAccount } = useDatabase();
  ...
  // Dynamic activation fee based on earnings
  const activationFee = getActivationFee(balance ?? 0);
    ...
    if (step === "form" && isValidPhone && profile?.id) {
      ...
      try {
        ...
        if (response.CheckoutRequestID) {
          ...
          // Payment successful - activate account in database
          const { success } = await activateAccount(phone, activationFee);
              ...
              <div className="space-y-5">
                <div className="bg-destructive/5 border border-destructive/20 rounded-2xl p-4 text-center">
                  ...
                  <p className="text-sm text-muted-foreground leading-relaxed">
                    ...
                    Based on your earnings of <span className="font-bold text-primary">KSH {(balance ?? 0).toLocaleString()}</span>, 
                    the activation fee is <span className="font-bold text-primary">KSH {activationFee}</span>.
                  </p>
                </div>
                ...
                <motion.button
                  ...
                  className="w-full h-14 rounded-2xl bg-primary text-primary-foreground font-bold text-base shadow-lg"
                >
                  Activate Now — KSH {activationFee}
                </motion.button>
            ...
            {step === "form" && (
              <div className="space-y-5">
                ...
                <motion.button
                  ...
                  className="w-full h-14 rounded-2xl bg-primary text-primary-foreground font-bold text-base shadow-lg disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  Send STK Push
                </motion.button>
